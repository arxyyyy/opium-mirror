...
import me.opium.features.gui.Component;
...
public class Slider
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.rect(context.getMatrices(), this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Opium.colorManager.getColorWithAlpha(Opium.moduleManager.getModuleByClass(UI.class).hoverAlpha.getValue()) : Opium.colorManager.getColorWithAlpha(Opium.moduleManager.getModuleByClass(UI.class).alpha.getValue()));
