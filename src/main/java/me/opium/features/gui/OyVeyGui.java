...
import me.opium.features.gui.items.buttons.ModuleButton;
...
public class OyVeyGui extends Screen {
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : Opium.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 100, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    Opium.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
