package me.opium.features.gui;

import me.opium.features.Feature;
import me.opium.features.gui.items.Item;
import me.opium.features.gui.items.buttons.Button;
import me.opium.features.modules.client.UI;
import me.opium.util.render.ColorUtil;
...
public class Component
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        int color = ColorUtil.toARGB(UI.getInstance().topRed.getValue(), UI.getInstance().topGreen.getValue(), UI.getInstance().topBlue.getValue(), 255);
        context.fill(this.x, this.y - 1, this.x + this.width, this.y + this.height - 6, UI.getInstance().rainbow.getValue() ? ColorUtil.rainbow(UI.getInstance().rainbowHue.getValue()).getRGB() : color);
        if (this.open) {
    ...
    protected void drawString(String text, double x, double y, int color) {
        context.drawTextWithShadow(Util.mc.textRenderer, text, (int) x, (int) y, color);
    }
}