...
public class ElementFriends extends Element {
    ...
    public void onRender2D(EventRender2D event) {
        ...
        ArrayList<PlayerEntity> friends = mc.world.getPlayers().stream()
                .filter(p -> Opium.FRIEND_MANAGER.isFriend(p.getName().getString()))
        ...
        RenderUtils.drawString(new MatrixStack(), this.name.getValue(), (int) this.frame.getX(), (int) this.frame.getY(), ModuleColor.getColor().getRGB());
        int index = 10;
        for (PlayerEntity player : friends) {
            RenderUtils.drawString(new MatrixStack(), player.getName().getString(), (int) this.frame.getX(), (int) (this.frame.getY() + index), ModuleHUDEditor.INSTANCE.getSecondColor().getColorIndex());
            index += 10;
        }
    }
