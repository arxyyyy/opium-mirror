package dev.opium.mod.gui.clickgui.components.impl;

import dev.opium.Opium;
import dev.opium.api.utils.math.Animation;
import dev.opium.mod.modules.impl.client.ClickGui;
import dev.opium.mod.modules.settings.impl.EnumSetting;
import dev.opium.core.impl.GuiManager;
import dev.opium.api.utils.render.Render2DUtil;
import dev.opium.api.utils.render.TextUtil;
import dev.opium.mod.gui.clickgui.components.Component;
import dev.opium.mod.gui.clickgui.ClickGuiScreen;
import dev.opium.mod.gui.clickgui.tabs.ClickGuiTab;
...
public class EnumComponent extends Component {
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		if (ClickGui.INSTANCE.mainEnd.booleanValue) {
			Render2DUtil.drawRectHorizontal(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : Opium.GUI.getColor(), ClickGui.INSTANCE.mainEnd.getValue());
		} else {
			Render2DUtil.drawRect(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : Opium.GUI.getColor());
		}
