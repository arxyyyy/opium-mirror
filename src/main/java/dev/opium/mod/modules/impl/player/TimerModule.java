...
public class TimerModule extends Module {
	...
	public void onRender2D(DrawContext drawContext, float tickDelta) {
		...
		if (MovementUtil.isMoving() && !Opium.PLAYER.insideBlock) {
			...
			if (timer2.passed(lastMs)) {
				...
				if (smooth.getValue()) {
					double timer = Opium.TIMER.getDefault() + (1 - end.ease(ease.getValue())) * (shiftTimer.getValueFloat() - 1) * (lastMs / accumulate.getValue());
					Opium.TIMER.set((float) Math.max(Opium.TIMER.getDefault(), timer));
				} else {
					Opium.TIMER.set(shiftTimer.getValueFloat());
				}
			}
		} else {
			if (moving) {
