...
import dev.opium.api.events.impl.LookAtEvent;
...
import dev.opium.api.utils.math.*;
import dev.opium.api.utils.render.ColorUtil;
import dev.opium.api.utils.render.JelloUtil;
...
import dev.opium.asm.accessors.IEntity;
import dev.opium.mod.modules.Module;
import dev.opium.mod.modules.impl.client.AntiCheat;
import dev.opium.mod.modules.impl.client.ClientSetting;
import dev.opium.mod.modules.impl.exploit.Blink;
import dev.opium.mod.modules.impl.player.PacketMine;
import dev.opium.mod.modules.impl.render.ExplosionSpawn;
...
import static dev.opium.api.utils.world.BlockUtil.getBlock;
import static dev.opium.api.utils.world.BlockUtil.hasCrystal;
...
public class AutoCrystal extends Module {
    ...
    public AutoCrystal() {
        ...
        setChinese("自动水晶");
        INSTANCE = this;
        Opium.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || Opium.ROTATION.inFov(entity.getPos(), fov.getValueFloat())) {
                    doPlace(crystalPos);
                }
            }
    ...
    private boolean faceVector(Vec3d directionVec) {
        if (!yawStep.getValue()) {
            ...
            if (Opium.ROTATION.inFov(directionVec, fov.getValueFloat())) {
                return true;
            }
        }
