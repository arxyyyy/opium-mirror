...
import dev.opium.api.events.impl.UpdateWalkingPlayerEvent;
import dev.opium.api.utils.combat.CombatUtil;
import dev.opium.api.utils.entity.EntityUtil;
import dev.opium.api.utils.entity.InventoryUtil;
import dev.opium.api.utils.math.ExplosionUtil;
import dev.opium.api.utils.math.FadeUtils;
import dev.opium.api.utils.math.Timer;
import dev.opium.api.utils.render.ColorUtil;
import dev.opium.api.utils.render.Render3DUtil;
import dev.opium.api.utils.world.BlockUtil;
import dev.opium.mod.modules.Module;
import dev.opium.mod.modules.settings.SwingSide;
import dev.opium.mod.modules.settings.impl.BooleanSetting;
import dev.opium.mod.modules.settings.impl.ColorSetting;
...
public class BedAura extends Module {
	...
	public void onEnable() {
		lastYaw = Opium.ROTATION.lastYaw;
		lastPitch = Opium.ROTATION.lastPitch;
	}
	...
	public void onRotate(RotateEvent event) {
		if (!rotate.getValue() && yawDeceive.getValue()) {
			...
			float[] newAngle = injectStep(Opium.ROTATION.getRotation(directionVec), steps.getValueFloat());
			lastYaw = newAngle[0];
	...
	private void doPlace(BlockPos pos) {
		...
		for (Direction i : Direction.values()) {
			...
			if ((legacy.getValue() && BlockUtil.canReplace(pos.offset(i)) || BlockUtil.clientCanPlace(pos.offset(i), false)) && BlockUtil.canClick(pos.offset(i).down()) && (!checkMine.getValue() || !Opium.BREAK.isMining(pos.offset(i)))) {
				facing = i;
				break;
			}
		}
	...
	private boolean canPlaceBed(BlockPos pos) {
		if (BlockUtil.canReplace(pos) && (!checkMine.getValue() || !Opium.BREAK.isMining(pos)) && (!legacy.getValue() || BlockUtil.canClick(pos.down()))) {
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
	...
	private boolean isTrueFacing(BlockPos pos, Direction facing) {
		...
		return Direction.fromRotation(Opium.ROTATION.getRotation(hitVec)[0]) == facing;
	}

	private boolean isTrueFacingNow(Direction facing) {
		return Direction.fromRotation(Opium.ROTATION.lastYaw) == facing;
	}
	public enum Page {
		General,
		Rotate,
	...
	public boolean faceVector(Vec3d directionVec) {
		if (!yawStep.getValue()) {
			Opium.ROTATION.lookAt(directionVec);
			return true;
		} else {
			this.directionVec = directionVec;
			if (Opium.ROTATION.inFov(directionVec, fov.getValueFloat())) return true;
		}
