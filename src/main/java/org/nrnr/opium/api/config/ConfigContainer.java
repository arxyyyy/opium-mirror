package org.nrnr.opium.api.config;
...
import org.nrnr.opium.api.config.setting.*;
        ...
        for (Field field : getClass().getDeclaredFields()) {
            if (Config.class.isAssignableFrom(field.getType())) {
                ...
                if (config == null) {
                    ...
                    Opium.error("Value for field {} is null!", field);
                    continue;
                }
                register(config);
            }
        ...
        if (jsonObj.has("configs")) {
            ...
            for (JsonElement je : element.getAsJsonArray()) {
                ...
                // couldn't parse Json value
                    ...
                    Opium.error("Couldn't parse Json for {}!", config.getName());
                    e.printStackTrace();
                }
            }
        }
