...
public class PathUtils {
    ...
    private PathUtils.PathResult findPath(
        ...
        if (iteration <= this.maxLength && (shortestPath == null || iteration <= shortestPath.size())) {
            for(Direction direction : Direction.values()) {
                if (mc.world.canPlace(this.placeState, pos.offset(direction).getBlockPos(), ShapeContext.absent())) {
                    if (list.contains(pos.offset(direction))) {
                        return new PathUtils.PathResult(list, pos.setPredictDirection(direction.getOpposite()));
                    }

                    if (!mc.world.getBlockState(pos.offset(direction).getBlockPos()).isReplaceable()) {
    ...
    public static class PathResult {
        ...
        public PathResult(List<PathBlock> list, PathUtils.PathBlock pos) {
            this.list = list;
            this.pos = pos;
        }
