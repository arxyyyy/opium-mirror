...
import org.nrnr.opium.impl.event.render.item.RenderFirstPersonEvent;
...
import org.nrnr.opium.mixin.accessor.HeldItemRendererAccessor;

public class SwingModule extends ToggleModule {
    public Config<Boolean> oldAnimationsM = new BooleanConfig("OldAnimations", "Older Animation Style", false);
    public Config<Boolean> oldAnimationsOff = new BooleanConfig("NewAnimations", "Newer Animation Style", false);
    public Config<Mode> mode = new EnumConfig<>("RendererMode", "The rendering mode for the Swing Module", Mode.Five, Mode.values());
    public Config<Boolean> slowAnimation = new BooleanConfig("SlowSwing", "Rotates to block before placing", true);
    public Config<Integer> slowAnimationVal = new NumberConfig<>("SlowSwingDelay", "The delay between each swing", 12, 20, 50);
    ...
    public void onTick(RunTickEvent event) {
        if (oldAnimationsM.getValue() && ((HeldItemRendererAccessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).getEquippedProgressMainHand() <= 1f) {
            ((HeldItemRendererAccessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setEquippedProgressMainHand(1f);
            if (!(mc.player == null)){
                ((HeldItemRendererAccessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setItemStackMainHand(mc.player.getMainHandStack());
            }
        }

        if (oldAnimationsOff.getValue() && ((HeldItemRendererAccessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).getEquippedProgressOffHand() <= 1f) {
            ((HeldItemRendererAccessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setEquippedProgressOffHand(1f);
            if (!(mc.player == null)){
                ((HeldItemRendererAccessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setItemStackOffHand(mc.player.getOffHandStack());
            }
        }
    }
    ...
    private void renderSwordAnimation(MatrixStack matrices, float f, float swingProgress, float equipProgress, Arm arm) {
        if (arm == Arm.LEFT && (mode.getValue() == Mode.Eleven || mode.getValue() == Mode.Ten || mode.getValue() == Mode.Nine || mode.getValue() == Mode.Three || mode.getValue() == Mode.Thirteen)) {
            ...
            matrices.translate(-Modules.VIEW_MODEL.positionXConfig.getValue(), Modules.VIEW_MODEL.positionYConfig.getValue(), Modules.VIEW_MODEL.positionZConfig.getValue());
    ...
    public void renderFirstPersonItemCustom(AbstractClientPlayerEntity player, float tickDelta, float pitch, Hand hand, float swingProgress, ItemStack item, float equipProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        if (!player.isUsingSpyglass()) {
            ...
            if (item.isOf(Items.CROSSBOW)) {
                ...
                RenderFirstPersonEvent event = new RenderFirstPersonEvent(hand, item, equipProgress, matrices);
